...
SslFilter::SslFilter( SSL_CTX *ctx ) {
...
ST SslFilter::new_inp( const char *data, ST size ) {
    return BIO_write( rbio, data, size );
}

ST SslFilter::new_out( const char *data, ST size ) {
...
ST SslFilter::get_inp( char *data, ST max_size ) {
    // try to read data
...
ST SslFilter::get_out( char *data, ST max_size ) {
